...
import org.kodigo.codemasters.services.IPatientService;
...
public class PatientControllerWebService {
    ...
    public ResponseEntity<Patient> register(@RequestBody Patient patient){
        Patient p = service.register(patient);
        
        URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(p.getIdPatient()).toUri();
    ...
    public ResponseEntity<List<Patient>> find(){
        List<Patient> list = service.find();
